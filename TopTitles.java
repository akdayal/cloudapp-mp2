...
// <<< Don't Change
    ...
    public static class TitleCountMap extends Mapper<Object, Text, Text, IntWritable> {
       ...
       private boolean isWordInStopWordsArray(String word, List<String> stopWordsArray) {
           for(String wd: stopWordsArray){
               if(wd.compareToIgnoreCase(word) == 0)
                return true;
           }
           return false;
        }
    ...
    public static class TopTitlesReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            while (itr.hasNext() && count != N + 1){
                 Pair p = (Pair)itr.next();
                 //send top N entry
                 context.write(new Text(p.second.toString()), new IntWritable(Integer.parseInt(p.first.toString())));
                 count ++;
            }
        }
    }
